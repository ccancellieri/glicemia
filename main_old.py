...
def parse_and_clean_data(file_path):
    ...
    la pulisce e la prepara per l'analisi.
    """
    ...
    header_row_index = -1
    header_keywords = ['Index', 'Date', 'Time', 'BG Reading', 'Bolus Volume Delivered']
    ...
    df = df[['Glicemia', 'Carboidrati', 'Bolo', 'Basale', 'Bolo_Correzione', 'ISF', 'IC']].copy()
    ...
    patient_info = {}
    try:
        patient_info['name'] = "Nuria Perez Diez" # Hardcoded per stabilità
...
def analyze_hypo_episodes(df):
    print("5. Analisi dettagliata episodi di ipoglicemia...")
    hypo_episodes = []
    in_hypo = False
...
def analyze_patterns_and_suggest(df):
    if df.empty: return pd.DataFrame()
    print("6. Analisi dei pattern e generazione suggerimenti...")
    analysis = []
    df_hourly = df.copy()
    ...
    print("7. Analisi completata.")
    return pd.DataFrame(analysis)

def create_interactive_excel_report(patient_info, df, sim_df, weekly_model_df, analysis_df, hypo_analysis_df, image_paths, output_file):
    ...
    with pd.ExcelWriter(output_file, engine='openpyxl') as writer:
        ...
        ws_param['K6'] = time(8, 0); ws_param['L6'] = "=B7"; ws_param['M6'] = "=C7"; ws_param['N6'] = "=D7"
        ws_param['K7'] = time(16, 0); ws_param['L7'] = "=B8"; ws_param['M7'] = "=C8"; ws_param['N7'] = "=D8"
        ws_param['K8'] = time(22, 0); ws_param['L8'] = "=B6"; ws_param['M8'] = "=C6"; ws_param['N8'] = "=D6"
        ws_param['K9'] = time(0, 0); ws_param['L9'] = "=B6"; ws_param['M9'] = "=C6"; ws_param['N9'] = "=D6"
        ...
        # --- Foglio Simulatore IOB (Dati Reali) ---
...
def main():
    ...
    for file in files_to_analyze:
        ...
        try:
            if analysis_df.empty: print("AVVISO: L'analisi dei pattern non ha prodotto risultati.")
            create_interactive_excel_report(patient_info, clean_df, simulation_df, weekly_model_df, analysis_df, hypo_analysis_df, image_paths, excel_output_file)
